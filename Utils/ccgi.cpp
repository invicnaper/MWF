...
#include <stdio.h>
    ...
    int mkstemp(char *tmpl)
    {
        int ret=-1;
        mktemp(tmpl);
        ret = open(tmpl, O_BINARY | O_RDWR | O_CREAT | O_EXCL, S_IRWALL);
        return ret;
    }
#else
    #include <unistd.h>
...
struct CGI_val {
    ...
    const char value[155];  /* variable value */
CGI_val();
};
...
struct CGI_varlist {
    ...
    const char varname[155];  /* variable name */
CGI_varlist();
};
...
urlencode(const char *in, char *out, const char *keep) {
 ...
 char *
CGI_urlencode(const char *in, char *out, const char *keep)
{
    return urlencode(in, out, keep);
}
...
CGI_get_post(CGI_varlist *v, const char *templatE) {
        ...
        (len = atoi(env)) > 0)
    {
        ...
        if (fread(buf, 1, len, stdin) == (unsigned int)len) {
            buf[len] = 0;
...
/* base64 conversion */
 ...
 const char b64encode[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    "abcdefghijklmnopqrstuvwxyz0123456789+/";

#define BAD 100

 const unsigned char b64decode[] = {
...
{
    char* out = CGI_encode_url(sString.c_str(), sKeep.c_str());
    string res(out);
    free(out);
    return res;
}

string EntityEncode(string sString)
{
    char* out = CGI_encode_entity(sString.c_str());
    string res(out);
    free(out);
    size_t pos = string::npos;
    while((pos = res.find("&#13;&#10;")) != string::npos) res.replace(pos, 10, "<br />");
    while((pos = res.find("&#13;")) != string::npos) res.replace(pos, 5, "<br />");
    while((pos = res.find("&#10;")) != string::npos) res.replace(pos, 5, "<br />");
    return res;
}
